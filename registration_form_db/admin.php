...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['mark_deleted'])) {
    ...
    if (!empty($idsToDelete)) {
        if (ConferenceRequest::markDeleted($dbo, $idsToDelete)) {
            $_SESSION['flash_message'] = 'Выбранные заявки помечены как удалённые.';
        } else {
...
$requests = ConferenceRequest::loadActive($dbo);
